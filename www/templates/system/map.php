			...
			<div class="outer">
				<div class="inner">
					...
					<div class="main">
						...
						<?php $this->renderMsgs(); ?>
						<div style="margin-left:3px; background-color: #373737; width:544px;">
							...
							<div>
								...
								<?php $map = $this->getData('map');
									...
									<?php for($j = 1; $j <= 15; $j++) :
											...
											?>
										<?php if ($line[$j]) : ?>
										<div style="position:absolute; top:<?php echo ($j-1)*32; ?>px; left:<?php echo ($x-1)*32; ?>px;" class="hasTip gd_<?php if ($case['usrId'] == $usr->id) { echo 'self'; } else if ($case['allyId'] == $ally->id) { echo 'ally'; } else { echo 'other'; } ?>" title="<?php echo $this->getData('X').':'.$this->getData('Y').':'.$case['z']; ?> - <?php echo $case['buildPoints']; ?> pts::<?php echo $case['usrName']; ?> (<?php echo $case['userPoints']; ?> pts)"><a href="p_town.php?id=<?php echo $case['twnId']; ?>"><img src="<?php echo SKIN.'imgs/'.$case['twnImg']; ?>.png" alt="" /></a></div>
										<?php else : ?>
										<div style="position:absolute; top:<?php echo ($j-1)*32; ?>px; left:<?php echo ($x-1)*32; ?>px;" class="hasTip <?php if ($grid[$x][$j]) { echo 'gd2_'. $grid[$x][$j]; } ?>" title="<?php echo $this->getData('X').':'.$this->getData('Y').':'.($x+($j-1)*17); ?>"><a><img src="<?php echo SKIN; ?>imgs/mempty.gif" alt="" /></a></div>
										<?php endif; ?>
